...
namespace System.Security.Permissions
{
  ...
  internal sealed class FileIOAccess
  {
    ...
    public bool IsSubsetOf(FileIOAccess operand)
    {
      ...
      if (!operand.m_allLocalFiles)
        return false;
      foreach (string path in this.m_set.UnsafeToStringArray())
      {
        string root = FileIOAccess.GetRoot(path);
        if (root == null || !FileIOAccess.IsLocalDrive(FileIOAccess.GetRoot(root)))
          return false;
      }
      return true;
    ...
    public override string ToString()
    {
      ...
      string str1 = "*AllLocalFiles*";
      string str2 = this.m_set.UnsafeToString();

...
namespace System.Text
{
  ...
  internal class ISCIIEncoding : EncodingNLS, ISerializable
  {
    ...
    private static byte[] SecondIndicByte = new byte[4]
    {
      (byte) 0,
      (byte) 233,
      (byte) 184,
    ...
    internal override unsafe int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, EncoderNLS baseEncoder)
    {
      ...
      if (isciiEncoder != null)
      {
        ...
        if (isciiEncoder.charLeftOver > char.MinValue)
        {
      ...
      while (encodingByteBuffer.MoreData)
      {
        ...
        {
          if (flag && (nextChar == '\x200C' || nextChar == '\x200D'))
          {
            if (nextChar == '\x200C')
            {
...
label_52:
      ...
      {
        if (((!isciiDecoder.MustFlush ? 1 : (ch2 > char.MinValue ? 1 : 0)) | (flag1 ? 1 : 0) | (flag3 ? 1 : 0)) != 0)
        {
    ...
    internal class ISCIIDecoder : DecoderNLS
    {
      ...
      internal override bool HasState
      {
        get
        {
          if (this.cLastCharForNextNukta == char.MinValue && this.cLastCharForNoNextNukta == char.MinValue && !this.bLastATR)
